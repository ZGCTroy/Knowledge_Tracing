...
def pre_process(root_dir, filename):
    ...
    df = pd.read_csv(
        ...
        header=0,
        usecols=['order_id', 'user_id', 'correct', 'skill_name','problem_id'],
        dtype={
            'user_id': 'category',
            'correct': 'int',
            'skill_name': 'category'
    ...
    for user_id in users_list:
        ...
        # 计算 skill正确率
        # user_df = calculate_and_add_correctness_ratio(user_df, mode='all')
        ...
        user_df = get_one_user_data(user_df, skill_num=skill_num,mode='all')
